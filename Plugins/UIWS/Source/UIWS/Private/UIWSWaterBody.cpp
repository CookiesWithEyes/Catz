...
{	
	...
	static ConstructorHelpers::FObjectFinder<UNiagaraSystem> nds(TEXT("/UIWS/ParticleEffects/Niagara/NS_MeshSplash.NS_MeshSplash"));
	if (nds.Succeeded())
	{
	...
	static ConstructorHelpers::FObjectFinder<UNiagaraSystem> nief(TEXT("/UIWS/ParticleEffects/Niagara/NS_MeshSplash.NS_MeshSplash"));
	if (nief.Succeeded())
	{
	...
	static ConstructorHelpers::FObjectFinder<UNiagaraSystem> ndef(TEXT("/UIWS/ParticleEffects/Niagara/NS_MeshSplashHit.NS_MeshSplashHit"));
	if (ndef.Succeeded())
	{
...
void AUIWSWaterBody::SplashAtlocation(FVector SplashLoc, FVector SplashVelocity, float SplashStrengthPercent)
{
	...
	if (NiagaraTemplate)
	{
		UNiagaraComponent* NiagaraSplash = UNiagaraFunctionLibrary::SpawnSystemAtLocation(this, NiagaraTemplate, (bOverrideParticleSettings ? (SplashLoc + InteractionEffectOffset) : SplashLoc), FRotator::ZeroRotator, FVector(UKismetMathLibrary::Lerp(InteractionEffectScaleMin, InteractionEffectScaleMax, SplashStrengthPercent)), true);
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("Cascade support will be removed in a future release.  Please upgrade your UIWS splash FX overrides to Niagara systems"));
	}
	++NiagaraWarningCounter;

	UParticleSystemComponent* Splash = UGameplayStatics::SpawnEmitterAtLocation(this, SysToSpawn, (bOverrideParticleSettings ? SplashLoc + InteractionEffectOffset : SplashLoc), FRotator::ZeroRotator, true);
...
void AUIWSWaterBody::PointDamageSplashAtlocation(FVector SplashLoc, float DamageAmount /*= 1.0f*/)
{
	...
	float DamageAlpha = FMath::GetMappedRangeValueClamped(FVector2D(0, DamageScale), FVector2D(0, 1), DamageAmount);
	...
	{
		UNiagaraComponent* NiagaraSplash = UNiagaraFunctionLibrary::SpawnSystemAtLocation(this, NiagaraTemplate, (bOverrideParticleSettings ? SplashLoc+ InteractionEffectOffset : SplashLoc), FRotator::ZeroRotator, FVector(ParticleScaleMult), true);
		NiagaraSplash->SetRenderCustomDepth(true);
		NiagaraSplash->SetCustomDepthStencilValue(1);
	...
	if (bEnableParticleOnDamage)
	{
		if (NiagaraWarningCounter < 1000 && NiagaraWarningCounter % 30 == 0)
		{
			UE_LOG(LogTemp, Warning, TEXT("Cascade support will be removed in a future release.  Please upgrade your UIWS Damage FX overrides to Niagara systems"));
		}
		++NiagaraWarningCounter;
...
void AUIWSWaterBody::RadialDamageSplashAtlocation(FVector SplashLoc, float DamageAmount /*= 100.0f*/)
{
	...
	if (NiagaraTemplate && bEnableParticleOnDamage)
	{
		UNiagaraComponent* NiagaraSplash = UNiagaraFunctionLibrary::SpawnSystemAtLocation(this, NiagaraTemplate, (bOverrideParticleSettings ? SplashLoc + DamageEffectOffset : SplashLoc), FRotator::ZeroRotator, FVector(ParticleScaleMult), true);
		NiagaraSplash->SetRenderCustomDepth(true);
		NiagaraSplash->SetCustomDepthStencilValue(1);
		NiagaraSplash->SetVariableFloat(DamageAlphaNiagaraParamName, DamageAlpha);
	...
	if (bEnableParticleOnDamage)
	{
		...
		++NiagaraWarningCounter;
		UParticleSystemComponent* Splash = UGameplayStatics::SpawnEmitterAtLocation(this, SysToSpawn, (bOverrideParticleSettings ? SplashLoc + DamageEffectOffset : SplashLoc), FRotator::ZeroRotator, true);
		Splash->SetWorldScale3D(FVector(ParticleScaleMult));
	}
	OnRadialDamageEffect(SplashLoc, RippleStrengthScaled, RippleSizeScaled, DamageAmount);
}
